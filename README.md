...
1. 植物分类学归属
2. 适合场景,会针对地栽,庭院,露台,阳台,窗台5个场景给出打分,最高10分最低1分,同时给出一个理由
3. 生长习性,就是介绍主要的生长特性,分为整体,花,果,叶,芽,枝,根,香
...
4. 环境要求,首先会介绍原生环境,然后会针对光照,温度,湿度,需肥度,抗性5个角度给出评价.

    + 光照,分为最低光照需要(光补偿点),最高光照耐受度(晒伤点而非光饱和点),最优光照强度和最优光照时长3个维度.由于植物生长分阶段,不同阶段的栽培阶段在一年中的时间范围不同,难度也不同所以这部分评价标准将是一个矩阵表其形式为,以春兰为例:
    ...
    + 适应温度范围,分为最高耐受温,最低耐受温,最大耐受温差,这些温度都是以至少3天内的最低温或者最高温为参考进行估计,而非准确的温度.我们统一使用摄氏度为单位
    + 适应湿度范围,分为最低耐受湿度,最高耐受湿度,这些湿度都是以至少3天内的最高湿度或者最低湿度为参考进行估计得到的值而非准确湿度,我们统一使用百分比为单位
    + 介质肥力要求:
        + 无肥植料,指纯颗粒石头无机物植料或者类似纯水水培这样的环境
        + 贫瘠植料,指保留少量有机质的植料以及类似板植的木板,或者低浓度营养液水培这类介质
        + 一般植料,指以腐殖质为主的植料
        + 肥沃植料,指以腐殖质为主且掺杂底肥的植料
    + 介质通透性要求:
        + 无要求
        + 保水透气
        + 疏水透气

5. 抗性,分为耐虫度,耐病度,耐涝度,耐旱度,耐肥度,耐瘠薄度,耐寒度,耐热度,耐晒度,这部分都是描述超过耐受范围后的抗性,都以0至10评分,这部分由于主要是主观感受所以只能说可以参考
...
7. 栽培方式描述,介绍主要的培育方式,会按节气区分介绍不同时间段中不同栽培方式所需的操作分为:
    + 土培,分为浇水方式,施肥方式,翻盆方式
        + 浇水方式
            + 干养,即盆面干透后至少1个月后再浇水
